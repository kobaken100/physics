...
Feynmanプロパゲータは、その形から、\\(x^0 > y^0\\) のときは、\\(y\\) で発生した粒子が \\(x\\) で消滅する確率振幅、\\(x^0 < y^0\\) のときは、\\(x\\) で発生した粒子が \\(y\\) で消滅する確率振幅を表すものに見える。ただし、今の段階では「そのように見える」程度だと思う。これを使って実際の計算をし、実験結果と一致する結果を出せることはあとで見る。
